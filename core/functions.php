...
function forwardTo($action_name){
	...
	include getCfg()->root_path."/ctrl.php";
	exit;
}

function redirectTo($action_name){
	header("Location: ".getCfg()->action_url.$action_name);
	exit;
}

function addRole($role){
	getCfg()->roles [$role] = true;
	$_SESSION['_roles'] = serialize(getCfg()->roles);
}

function inRole($role){
	return isset(getCfg()->roles[$role]);
}
?>